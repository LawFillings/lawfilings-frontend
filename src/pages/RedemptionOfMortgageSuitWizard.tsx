...
const caseType = caseTypes.find((ct) => ct.id === 'ct-suit-redemption-mortgage')!;
...
export function RedemptionOfMortgageSuitWizard({
  onBack,
  onOpenPricing,
  ...
  const [mortgageeInPossession, setMortgageeInPossession] = useState(saved?.mortgageeInPossession ?? false);
  const [amountDueNow, setAmountDueNow] = useState(saved?.amountDueNow ?? '');
  ...
  const handleSaveDraft = async () => {
    ...
    const content: SavedContent & { [WIZARD_CASE_TYPE_KEY]: string } = {
      ...
      verificationPlace,
      documentEntries,
      [WIZARD_CASE_TYPE_KEY]: 'ct-suit-redemption-mortgage',
    };
    try {
      if (caseId && draftId) {
        ...
        const created = await casesClient.createCase(
          {
            title: `${plaintiffName || 'Plaintiff'} vs. ${defendantName || 'Defendant'} — Suit for Redemption of Mortgage`,
  ...
  const citationMatches = findFixedCaseTypeCitation('ct-suit-redemption-mortgage');
  const caseLawMatches = findFixedCaseTypeCaseLaw('ct-suit-redemption-mortgage');
  ...
  const draftSections: DraftSection[] = [
    {
      ...
      paragraphs: [
        toThatClause(
          `by ${
            mortgageDeedDetails.trim() || '[mortgage deed details — date, registration number, and parties]'
          }, the Plaintiff mortgaged to the Defendant the property described as ${
            ...
            mortgageeInPossession ? ', and the Defendant has, since the mortgage, been in possession of the mortgaged property' : ''
          }`
        ),
      ],
    ...
    {
      heading: 'Facts',
      paragraphs: [toThatClause(factsNarrative.trim() || '[Describe the circumstances — any tender of the mortgage money already made, and how/why the Defendant has refused to accept it or release the property/documents]')],
      incomplete: !factsNarrative.trim(),
      role: 'facts',
    },
    {
      heading: "The Plaintiff's right to redeem",
      paragraphs: [
        toThatClause(
          `the Plaintiff is ready and willing to pay the Defendant the sum of ${
            amountDueNow.trim() || '[amount now due]'
          }, being the amount genuinely due under the mortgage, and is entitled, under Section 60 of the Transfer of Property Act, 1882, to require the Defendant, upon such payment, to deliver up the mortgage-deed and all documents relating to the mortgaged property${
            mortgageeInPossession ? ', to deliver possession of the mortgaged property to the Plaintiff,' : ''
          } and to execute and (where necessary) register an acknowledgment that the Defendant's interest in the property has been extinguished`
        ),
      ],
      incomplete: !amountDueNow.trim(),
      role: 'grounds',
    },
    ...
    },
    {
      heading: 'Prayer',
      paragraphs: [
        `It is therefore most respectfully prayed that this Hon'ble Court may be pleased to pass a decree of redemption directing that, upon payment by the Plaintiff of ${
          amountDueNow.trim() || '[amount now due]'
        } to the Defendant, the Defendant shall (a) deliver to the Plaintiff the mortgage-deed and all documents relating to the mortgaged property; ${
          mortgageeInPossession ? '(b) deliver possession of the mortgaged property to the Plaintiff; (c)' : '(b)'
        } execute, and where necessary have registered, an acknowledgment that the Defendant's interest in the mortgaged property has been extinguished; and pass any other order(s) as this Hon'ble Court may deem fit and proper in the interest of justice.`,
      ],
    },
  ...
  return (
    <div>
      ...
      >
        ...
        {step === 2 && (
          <div>
            ...
            <DocumentAutofill
              ...
              fields={[
                { key: 'plaintiffName', label: 'the mortgagor (the borrower who mortgaged the property)', value: plaintiffName, set: setPlaintiffName },
                { key: 'plaintiffAge', label: 'Age of the mortgagor (the borrower who mortgaged the property)', kind: 'amount', hint: 'digits only', value: plaintiffAge, set: setPlaintiffAge },
                { key: 'plaintiffAddress', label: 'Address of the mortgagor (the borrower who mortgaged the property)', value: plaintiffAddress, set: setPlaintiffAddress },
                { key: 'defendantName', label: 'the mortgagee (the lender who holds the mortgage)', value: defendantName, set: setDefendantName },
                { key: 'defendantAddress', label: 'Address of the mortgagee (the lender who holds the mortgage)', value: defendantAddress, set: setDefendantAddress },
            ...
            <div className="form-grid">
              <label className="form-field">
                <span>{mode === 'advocate' ? 'Plaintiff (mortgagor)' : 'Your name'}</span>
              ...
              <label className="form-field">
                <span>{mode === 'advocate' ? 'Defendant (mortgagee)' : 'Other party'}</span>
            ...
            <label className="form-field" style={{ marginTop: 'var(--space-4)' }}>
              <span>
                <input type="checkbox" checked={mortgageeInPossession} onChange={(e) => setMortgageeInPossession(e.target.checked)} style={{ marginRight: 'var(--space-2)' }} />
                The Defendant (mortgagee) is in possession of the mortgaged property
              </span>
            </label>
          </div>
        )}
        ...
        {step === 3 && (
          <div>
            <h3 className="step-heading">Facts and the amount now due</h3>
            <label className="form-field">
              <span>Facts — circumstances of the mortgage, any tender already made, and the Defendant's response</span>
              <textarea
                className="facts-textarea"
                rows={5}
                value={factsNarrative}
                onChange={(e) => setFactsNarrative(e.target.value)}
                placeholder="Describe how the mortgage came about, any payments already made towards it, and — if you've already offered to pay the balance — what the Defendant said or did in response"
              />
            </label>
            <label className="form-field" style={{ marginTop: 'var(--space-4)', maxWidth: 320 }}>
              <span>Amount now due to redeem</span>
              <input type="text" value={amountDueNow} onChange={(e) => setAmountDueNow(e.target.value)} placeholder="₹" />
            </label>
            <label className="form-field" style={{ marginTop: 'var(--space-4)', maxWidth: 320 }}>
            ...
            <p className="step-help">
              A term in the mortgage deed that tries to cut off or unreasonably restrict your right to redeem is void
              and can be ignored — courts have long held that "once a mortgage, always a mortgage" (
              <em>Ganga Dhar v. Shankar Lal</em>, AIR 1958 SC 770).
            </p>
            {user ? (
        ...
        {step === 5 && (
          <div>
            ...
            <p className="step-help">
              Add each document you're annexing — typically a copy of the mortgage deed, proof of any prior
              payments, and correspondence around any tender already made — in the order it will be paginated.
            </p>
        ...
        {step === 6 && (
          <div>
            ...
            <DraftDocument title="Suit for Redemption of Mortgage — Index" causeTitleHtml={indexCauseTitleHtml} sections={indexSections} />
            ...
            <DraftDocument
              ...
              subtitle={`Suit for Redemption of Mortgage — ${plaintiffName || '[Plaintiff]'} vs. ${defendantName || '[Defendant]'}`}
              causeTitleHtml={causeTitleHtml}
              sections={applyJudgeStyleToSections(draftSections, judgeStyleProfile)}
            />
            <h4 style={{ marginTop: 'var(--space-6)' }}>Part III — Affidavit</h4>
            <DraftDocument title="Suit for Redemption of Mortgage — Affidavit" causeTitleHtml={affidavitCauseTitleHtml} sections={affidavitSections} />
