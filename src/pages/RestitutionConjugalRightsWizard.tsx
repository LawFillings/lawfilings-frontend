...
const caseType = caseTypes.find((ct) => ct.id === 'ct-restitution-conjugal-rights')!;
...
export function RestitutionConjugalRightsWizard({
  onBack,
  onOpenPricing,
  ...
  const handleSaveDraft = async () => {
    ...
    const content: SavedContent & { [WIZARD_CASE_TYPE_KEY]: string } = {
      ...
      verificationPlace,
      documentEntries,
      [WIZARD_CASE_TYPE_KEY]: 'ct-restitution-conjugal-rights',
    };
    try {
      if (caseId && draftId) {
        ...
        const created = await casesClient.createCase(
          {
            title: `${petitionerName || 'Petitioner'} vs. ${respondentName || 'Respondent'} — Restitution of Conjugal Rights`,
  ...
  const citationMatches = findFixedCaseTypeCitation('ct-restitution-conjugal-rights');
  ...
  const draftSections: DraftSection[] = [
    ...
    {
      heading: 'Facts constituting withdrawal from society',
      paragraphs: [
        toThatClause(
          factsNarrative.trim() ||
            '[Describe when and how the Respondent withdrew from the Petitioner\'s society, and why there was no reasonable excuse for doing so]'
        ),
      ],
      ...
      : []),
    {
      heading: 'Prayer',
      paragraphs: [
        `It is therefore most respectfully prayed that this Hon'ble Court may be pleased to pass a decree for restitution of conjugal rights, directing the Respondent to resume cohabitation with the Petitioner, and pass any other order(s) as this Hon'ble Court may deem fit and proper in the interest of justice.`,
      ],
    },
  ...
  return (
    <div>
      ...
      >
        ...
        {step === 1 && (
          <div>
            <h3 className="step-heading">Facts constituting withdrawal from society</h3>
            <p className="step-help">
              Explain when and how the Respondent withdrew from the Petitioner's company, and why there was no
              reasonable excuse for it — this is the entire ground under Section 9.
            </p>
            <textarea
              className="facts-textarea"
              rows={6}
              ...
              placeholder="Describe when the Respondent left, the circumstances, and why the Petitioner believes there was no reasonable excuse"
            />
            {user ? (
        ...
        {step === 4 && (
          <div>
            ...
            <DraftDocument title="Restitution of Conjugal Rights — Index" causeTitleHtml={indexCauseTitleHtml} sections={indexSections} />
            <h4 style={{ marginTop: 'var(--space-6)' }}>Part II — Petition</h4>
            <DraftDocument
              title="Restitution of Conjugal Rights Petition"
              subtitle={`Petition under Section 9, Hindu Marriage Act, 1955 — ${petitionerName || '[Petitioner]'} vs. ${respondentName || '[Respondent]'}`}
              causeTitleHtml={causeTitleHtml}
              sections={applyJudgeStyleToSections(draftSections, judgeStyleProfile)}
            />
            <h4 style={{ marginTop: 'var(--space-6)' }}>Part III — Affidavit</h4>
            <DraftDocument title="Restitution of Conjugal Rights — Affidavit" causeTitleHtml={affidavitCauseTitleHtml} sections={affidavitSections} />
