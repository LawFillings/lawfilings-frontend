...
const caseType = caseTypes.find((ct) => ct.id === 'ct-succession-certificate')!;
...
const COURT_LEVEL_OPTIONS: { id: CourtLevel; label: string; help: string }[] = [
  {
    ...
    help: 'The usual forum, wherever the deceased ordinarily resided or had property.',
  },
  {
    id: 'high_court',
...
interface SavedContent {
  ...
  rightClaimed: string;
  debtsSecurities: string;
  otherHeirsNote: string;
  advocateName: string;
...
export function SuccessionCertificateWizard({
  onBack,
  onOpenPricing,
  ...
  const [debtsSecurities, setDebtsSecurities] = useState(saved?.debtsSecurities ?? '');
  const [otherHeirsNote, setOtherHeirsNote] = useState(saved?.otherHeirsNote ?? '');
  ...
  const handleSaveDraft = async () => {
    ...
    const content: SavedContent & { [WIZARD_CASE_TYPE_KEY]: string } = {
      ...
      verificationPlace,
      documentEntries,
      [WIZARD_CASE_TYPE_KEY]: 'ct-succession-certificate',
    };
    try {
      if (caseId && draftId) {
        ...
        const created = await casesClient.createCase(
          {
            title: `${petitionerName || 'Petitioner'} — Succession Certificate re. estate of ${deceasedName || 'Deceased'}`,
  ...
  const citationMatches = findFixedCaseTypeCitation('ct-succession-certificate');
  const caseLawMatches = findFixedCaseTypeCaseLaw('ct-succession-certificate');
  ...
  const draftSections: DraftSection[] = [
    {
      ...
      paragraphs: [
        toThatClause(
          `${deceasedName || '[Name of the deceased]'} (hereinafter "the deceased") died on ${
            dateOfDeath || '[date of death]'
          }, and at the time of his/her death ordinarily resided at ${
            ordinaryResidence || '[ordinary residence of the deceased at the time of death]'
    ...
    {
      heading: 'Family and other near relatives of the deceased',
      paragraphs: [
        toThatClause(
          familyRelatives.trim() ||
            "[List the deceased's family or other near relatives and their respective residences]"
        ),
      ],
    ...
    },
    {
      ...
      paragraphs: [
        toThatClause(
          rightClaimed.trim() ||
            '[State the right in which the Petitioner claims, e.g. as a Class I legal heir of the deceased under the applicable law of succession]'
        ),
      ],
      incomplete: !rightClaimed.trim(),
    },
    {
      heading: 'Debts and securities in respect of which the certificate is applied for',
      paragraphs: [
        toThatClause(
          debtsSecurities.trim() ||
            '[Describe the debts and securities — e.g. bank account numbers and balances, shares, insurance proceeds — in respect of which the certificate is sought]'
        ),
      ],
      incomplete: !debtsSecurities.trim(),
    },
    {
      heading: 'Absence of impediment',
      paragraphs: [
        toThatClause(
          'there is no impediment under section 370 of the Indian Succession Act, 1925, or under any other provision of the said Act or any other enactment, to the grant of the certificate applied for, or to the validity thereof if granted.'
        ),
      ],
    },
    ...(otherHeirsNote.trim()
      ? [
          {
            heading: 'Other legal heirs',
            paragraphs: [toThatClause(otherHeirsNote.trim())],
          },
        ]
      : []),
      ...
      : []),
    {
      heading: 'Prayer',
      paragraphs: [
        `It is therefore most respectfully prayed that this Hon'ble ${courtNoun} may be pleased to grant a Succession Certificate under the Indian Succession Act, 1925, in favour of the Petitioner, authorising the Petitioner to receive, collect, and give valid discharges in respect of the debts and securities set out above, and to pass any other order(s) as this Hon'ble ${courtNoun} may deem fit and proper in the interest of justice.`,
      ],
    },
  ...
  const causeTitleInfo = {
    ...
    respondentName: '',
    noRespondent: true,
    caseNumberLine: `Succession Case No. _____ of ${new Date().getFullYear()}`,
  ...
  return (
    <div>
      ...
      >
        {step === 0 && (
          <div>
            ...
            <p className="step-help">
              A succession certificate covers only debts and securities of the deceased — not immovable property,
              and not a will's other assets (those need Probate or Letters of Administration instead).
            </p>
        ...
        {step === 1 && (
          <div>
            <h3 className="step-heading">Deceased and claim details</h3>
            <div className="form-grid">
              <label className="form-field">
              ...
              <label className="form-field">
                <span>Deceased's ordinary residence at the time of death</span>
            ...
            <label className="form-field" style={{ marginTop: 'var(--space-4)' }}>
              <span>Family or other near relatives of the deceased, and their respective residences</span>
              <textarea
            ...
            <label className="form-field" style={{ marginTop: 'var(--space-4)' }}>
              ...
              <textarea
                ...
                placeholder="e.g. as the son and one of the Class I legal heirs of the deceased under the Hindu Succession Act, 1956"
              />
            </label>
            <label className="form-field" style={{ marginTop: 'var(--space-4)' }}>
              <span>Debts and securities the certificate is sought for</span>
              <textarea
                className="facts-textarea"
                rows={4}
                value={debtsSecurities}
                onChange={(e) => setDebtsSecurities(e.target.value)}
                placeholder="e.g. Savings Account No. ... with [Bank], balance ₹...; 100 equity shares of [Company]; Life insurance policy No. ... with [Insurer]"
              />
            </label>
            <label className="form-field" style={{ marginTop: 'var(--space-4)' }}>
              <span>Other legal heirs (optional — note their consent, or that they're being impleaded/served)</span>
              <textarea
                className="facts-textarea"
                rows={3}
                value={otherHeirsNote}
                onChange={(e) => setOtherHeirsNote(e.target.value)}
                placeholder="e.g. the other legal heirs of the deceased have no objection to the grant of this certificate to the Petitioner, and their no-objection is annexed hereto"
              />
            </label>
            {user ? (
        ...
        {step === 4 && (
          <div>
            ...
            <DraftDocument title="Succession Certificate Petition — Index" causeTitleHtml={indexCauseTitleHtml} sections={indexSections} />
            <h4 style={{ marginTop: 'var(--space-6)' }}>Part II — Petition</h4>
            <DraftDocument
              title="Succession Certificate Petition"
              subtitle={`Petition under Section 372, The Indian Succession Act, 1925 — re. estate of ${
            ...
            <DraftDocument title="Succession Certificate Petition — Affidavit" causeTitleHtml={affidavitCauseTitleHtml} sections={affidavitSections} />
