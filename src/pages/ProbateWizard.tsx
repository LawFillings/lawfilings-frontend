...
const caseType = caseTypes.find((ct) => ct.id === 'ct-probate')!;
...
const COURT_LEVEL_OPTIONS: { id: CourtLevel; label: string; help: string }[] = [
  {
    ...
    help: 'The usual forum, wherever the testator had a fixed place of abode or property.',
  },
  {
    id: 'high_court',
...
export function ProbateWizard({
  ...
  const [testatorName, setTestatorName] = useState(saved?.testatorName ?? '');
  ...
  const [willDate, setWillDate] = useState(saved?.willDate ?? '');
  ...
  const handleSaveDraft = async () => {
    ...
    const content: SavedContent & { [WIZARD_CASE_TYPE_KEY]: string } = {
      ...
      verificationPlace,
      documentEntries,
      [WIZARD_CASE_TYPE_KEY]: 'ct-probate',
    };
    try {
      if (caseId && draftId) {
        ...
        const created = await casesClient.createCase(
          {
            title: `${petitionerName || 'Petitioner'} — Probate of the Will of ${testatorName || 'Testator'}`,
  ...
  const citationMatches = findFixedCaseTypeCitation('ct-probate');
  const caseLawMatches = findFixedCaseTypeCaseLaw('ct-probate');
  ...
  const draftSections: DraftSection[] = [
    {
      heading: 'Particulars of the testator and the Will',
      paragraphs: [
        toThatClause(
          `${testatorName || '[Name of the testator]'} (hereinafter "the testator") died on ${
            dateOfDeath || '[date of death]'
          }, and at the time of his/her death had a fixed place of abode, or property, at ${
            ordinaryResidence || '[address within the jurisdiction of this Hon\'ble Court]'
          }, within the jurisdiction of this Hon'ble ${courtNoun}.`
        ),
        toThatClause(
          `the testator executed his/her last Will and testament dated ${
            willDate || '[date of the Will]'
          }, and the writing annexed hereto is the testator's said last Will and testament, duly executed by the testator.`
        ),
        toThatClause(
          `the Petitioner is the executor named in the said Will, and is accordingly entitled to apply for and be granted Probate thereof.`
        ),
      ],
      incomplete: !testatorName || !dateOfDeath || !willDate,
    },
    {
      ...
      paragraphs: [
        toThatClause(
          assetsDescription.trim() ||
            '[Describe the amount and nature of the assets likely to come to the Petitioner\'s hands as executor]'
        ),
      ],
      ...
      : []),
    {
      heading: 'Prayer',
      paragraphs: [
        `It is therefore most respectfully prayed that this Hon'ble ${courtNoun} may be pleased to grant Probate of the last Will and testament dated ${
          willDate || '[date of the Will]'
        } of the deceased testator ${testatorName || '[Testator]'} to the Petitioner, and to pass any other order(s) as this Hon'ble ${courtNoun} may deem fit and proper in the interest of justice.`,
      ],
    },
  ...
  const causeTitleInfo = {
    ...
    respondentName: '',
    noRespondent: true,
    caseNumberLine: `Probate Case No. _____ of ${new Date().getFullYear()}`,
  ...
  const affidavitSections: DraftSection[] = [
    ...
    },
    {
      unnumbered: true,
      paragraphs: [
        '1. That I am the Petitioner in the present case, being the executor named in the last Will and testament of the deceased testator, and I am well conversant with the facts and circumstances of the case.',
      ...
      >
        {step === 0 && (
          <div>
            ...
            <p className="step-help">
              Probate can only be granted to the executor named in the Will — if there's no surviving named
              executor, use Letters of Administration (with the Will annexed) instead.
            </p>
            ...
            <div className="form-grid">
              <label className="form-field">
                <span>
                  {mode === 'advocate' ? 'Petitioner (executor)' : 'Your name (you are the executor)'}
                </span>
          ...
          <div>
            ...
            <div className="form-grid">
              <label className="form-field">
                ...
                <input type="text" value={testatorName} onChange={(e) => setTestatorName(e.target.value)} />
              </label>
              <label className="form-field">
                <span>Date of the testator's death</span>
                <input type="date" value={dateOfDeath} onChange={(e) => setDateOfDeath(e.target.value)} />
              </label>
              <label className="form-field">
                <span>Testator's fixed place of abode / property (within the Court's jurisdiction)</span>
                <input type="text" value={ordinaryResidence} onChange={(e) => setOrdinaryResidence(e.target.value)} />
              </label>
              <label className="form-field">
                <span>Date the Will was executed</span>
                <input type="date" value={willDate} onChange={(e) => setWillDate(e.target.value)} />
              </label>
            </div>
            <label className="form-field" style={{ marginTop: 'var(--space-4)' }}>
              <span>Assets likely to come to the Petitioner's hands as executor</span>
              <textarea
        ...
        {step === 3 && (
          <div>
            ...
            <p className="step-help">
              Add each document you're annexing, in the order it will be paginated — including the original Will,
              the death certificate of the testator, and proof of the testator's residence or property.
            </p>
        ...
        {step === 4 && (
          <div>
            ...
            <DraftDocument title="Probate Petition — Index" causeTitleHtml={indexCauseTitleHtml} sections={indexSections} />
            <h4 style={{ marginTop: 'var(--space-6)' }}>Part II — Petition</h4>
            <DraftDocument
              title="Probate Petition"
              subtitle={`Petition under Section 276, The Indian Succession Act, 1925 — re. the Will of ${
            ...
            <DraftDocument title="Probate Petition — Affidavit" causeTitleHtml={affidavitCauseTitleHtml} sections={affidavitSections} />
