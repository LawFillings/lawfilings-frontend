...
const caseType = caseTypes.find((ct) => ct.id === 'ct-suit-partition')!;
...
interface SavedContent {
  ...
  defendantEntries: string[];
  defendantAddress: string;
  propertyDescription: string;
  relationshipToProperty: string;
  shareClaimed: string;
  priorPartition: 'yes' | 'no' | null;
...
export function PartitionSuitWizard({
  onBack,
  onOpenPricing,
  ...
  const [defendantEntries, setDefendantEntries] = useState<string[]>(saved?.defendantEntries ?? ['']);
  ...
  const [relationshipToProperty, setRelationshipToProperty] = useState(saved?.relationshipToProperty ?? '');
  const [shareClaimed, setShareClaimed] = useState(saved?.shareClaimed ?? '');
  const [priorPartition, setPriorPartition] = useState<'yes' | 'no' | null>(saved?.priorPartition ?? null);
  ...
  const addDefendant = () => setDefendantEntries((d) => [...d, '']);
  const removeDefendant = (i: number) => setDefendantEntries((d) => (d.length > 1 ? d.filter((_, idx) => idx !== i) : d));
  const updateDefendant = (i: number, value: string) =>
    setDefendantEntries((d) => d.map((entry, idx) => (idx === i ? value : entry)));
  ...
  const filledDefendants = defendantEntries.filter((d) => d.trim().length > 0);
  ...
  const handleSaveDraft = async () => {
    ...
    const content: SavedContent & { [WIZARD_CASE_TYPE_KEY]: string } = {
      ...
      verificationPlace,
      documentEntries,
      [WIZARD_CASE_TYPE_KEY]: 'ct-suit-partition',
    };
    try {
      if (caseId && draftId) {
        ...
        const created = await casesClient.createCase(
          {
            title: `${plaintiffName || 'Plaintiff'} vs. ${filledDefendants[0] || 'Defendants'} — Suit for Partition`,
  ...
  const draftSections: DraftSection[] = [
    {
      ...
      paragraphs: [
        toThatClause(
          ...
          }, which is presently held jointly by the Plaintiff and the Defendant(s), ${defendantListProse}.`
        ),
      ],
      incomplete: !propertyDescription.trim() || !shareClaimed.trim(),
    },
    {
      heading: 'Facts constituting the cause of action',
      paragraphs: [toThatClause(factsNarrative.trim() || '[Describe the facts leading to this suit, including why an amicable partition has not been possible]')],
      incomplete: !factsNarrative.trim(),
      role: 'facts',
    },
    {
      heading: 'No prior partition',
      paragraphs: [
        toThatClause(
          priorPartition === 'no'
            ? 'the property described above has not been partitioned by metes and bounds, by any registered deed of partition, or by any decree of a court, and continues to be held jointly by the parties, within the meaning of section 6(5) of the Hindu Succession Act, 1956'
            : priorPartition === 'yes'
              ? '[This property may already stand partitioned — go back and reconsider whether a fresh partition suit lies, per section 6(5) of the Hindu Succession Act, 1956]'
              : '[Confirm whether the property has already been partitioned]'
        ),
      ],
      incomplete: priorPartition !== 'no',
    },
    ...
    },
    {
      heading: 'Prayer',
      paragraphs: [
        `It is therefore most respectfully prayed that this Hon'ble Court may be pleased to pass a preliminary decree declaring the Plaintiff entitled to a ${
          shareClaimed.trim() || '[share]'
        } share in the suit property, and thereafter a final decree effecting actual partition and separate possession of the Plaintiff's share by metes and bounds, or, where the property cannot be conveniently so divided, by sale of the property and division of the proceeds, and pass any other order(s) as this Hon'ble Court may deem fit and proper in the interest of justice.`,
      ],
      incomplete: !shareClaimed.trim(),
    },
  ...
  const causeTitleInfo = {
    ...
    respondentName: filledDefendants[0] || '',
    respondentEntries: filledDefendants.length > 1 ? filledDefendants : undefined,
  ...
  return (
    <div>
      ...
      >
          ...
          <div>
            ...
            {selectedState ? (
              <LocationSelector
                ...
                label="District"
                helpText={`Districts of ${selectedState.label}. A partition suit over immovable property is filed where the property is situated.`}
        ...
        {step === 2 && (
          <div>
            ...
            <div className="form-grid">
              ...
              <label className="form-field">
                <span>Defendants' (shared) address</span>
            ...
            <h3 className="step-heading" style={{ marginTop: 'var(--space-5)' }}>
              {mode === 'advocate' ? 'Defendants (other co-sharers)' : 'The other co-owners'}
            </h3>
            <p className="step-help">Add every other co-sharer of the property — they must all be made parties to a partition suit.</p>
            {defendantEntries.map((d, i) => (
              <div key={i} style={{ display: 'flex', gap: 'var(--space-3)', alignItems: 'flex-end', marginBottom: 'var(--space-3)' }}>
                <label className="form-field" style={{ flex: 1 }}>
                  <span>Defendant No. {i + 1}</span>
                  <input type="text" value={d} onChange={(e) => updateDefendant(i, e.target.value)} />
                </label>
                {defendantEntries.length > 1 && (
            ...
            <label className="form-field" style={{ marginTop: 'var(--space-5)' }}>
              <span>Describe the property to be partitioned</span>
              <textarea
                className="facts-textarea"
                rows={3}
                value={propertyDescription}
                onChange={(e) => setPropertyDescription(e.target.value)}
                placeholder="e.g. the ancestral immovable property bearing House No. 12, Model Town, together with the agricultural land bearing Khasra No. 45"
              />
            </label>
            <label className="form-field" style={{ marginTop: 'var(--space-4)' }}>
              <span>How is the Plaintiff related to this property?</span>
              <textarea
                className="facts-textarea"
                rows={2}
                value={relationshipToProperty}
                onChange={(e) => setRelationshipToProperty(e.target.value)}
                placeholder="e.g. a daughter of late Sh. Ram Lal, and a coparcener in the Hindu Undivided Family owning the property"
              />
            </label>
          </div>
        )}
        ...
        {step === 3 && (
          <div>
            <h3 className="step-heading">No prior partition</h3>
            <p className="step-help">
              Under section 6(5) of the Hindu Succession Act, 1956, "partition" means a registered partition deed or
              a court decree — an oral or informal family arrangement does not count. If the property has already
              been partitioned that way, a fresh partition suit generally won't lie.
            </p>
            <div className="grounds-grid">
              <button
                className={priorPartition === 'no' ? 'ground-card active' : 'ground-card'}
                onClick={() => setPriorPartition('no')}
              >
                No — the property has never been formally partitioned
              </button>
              <button
                className={priorPartition === 'yes' ? 'ground-card active' : 'ground-card'}
                onClick={() => setPriorPartition('yes')}
              >
                Yes / not sure
              </button>
            </div>
            {priorPartition === 'yes' && (
              <p className="step-help" style={{ marginTop: 'var(--space-3)', color: 'var(--status-danger-text, #b91c1c)' }}>
                If the property already stands partitioned by a registered deed or court decree, this suit may not
                be maintainable — consider getting advice before proceeding.
              </p>
            )}
            {user ? (
        ...
        {step === 4 && (
          <div>
            <h3 className="step-heading">Facts and share claimed</h3>
            <label className="form-field">
              ...
              <textarea
                ...
                placeholder="Describe how the property came to be jointly held, and why an amicable partition has not been possible"
              />
            </label>
            <label className="form-field" style={{ marginTop: 'var(--space-4)', maxWidth: 320 }}>
              <span>Share claimed by the Plaintiff</span>
              <input type="text" value={shareClaimed} onChange={(e) => setShareClaimed(e.target.value)} placeholder="e.g. one-fourth (1/4th)" />
            </label>
            <label className="form-field" style={{ marginTop: 'var(--space-4)', maxWidth: 320 }}>
            ...
            <p className="step-help">
              A partition suit is usually valued notionally (a fraction of the property's market value, or a fixed
              fee) rather than at the full property value, under your state's Court Fees Act — use the Court Fee
              Calculator on this platform, or confirm with the filing registry.
            </p>
          </div>
        )}
        ...
        {step === 7 && (
          <div>
            ...
            <DraftDocument title="Suit for Partition — Index" causeTitleHtml={indexCauseTitleHtml} sections={indexSections} />
            ...
            <DraftDocument
              ...
              subtitle={`Suit for Partition — ${plaintiffName || '[Plaintiff]'} vs. ${defendantListProse}`}
              causeTitleHtml={causeTitleHtml}
              sections={applyJudgeStyleToSections(draftSections, judgeStyleProfile)}
            />
            <h4 style={{ marginTop: 'var(--space-6)' }}>Part III — Affidavit</h4>
            <DraftDocument title="Suit for Partition — Affidavit" causeTitleHtml={affidavitCauseTitleHtml} sections={affidavitSections} />
