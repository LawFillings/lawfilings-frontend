...
const caseType = caseTypes.find((ct) => ct.id === 'ct-suit-cancellation-of-document')!;

type GroundType = 'void' | 'voidable';
...
export function CancellationOfDocumentSuitWizard({
  onBack,
  onOpenPricing,
  ...
  const [documentDetails, setDocumentDetails] = useState(saved?.documentDetails ?? '');
  const [groundType, setGroundType] = useState<GroundType>(saved?.groundType ?? 'voidable');
  const [groundsNarrative, setGroundsNarrative] = useState(saved?.groundsNarrative ?? '');
  const [apprehendedInjury, setApprehendedInjury] = useState(saved?.apprehendedInjury ?? '');
  const [knowledgeDate, setKnowledgeDate] = useState(saved?.knowledgeDate ?? '');
  const [isRegistered, setIsRegistered] = useState(saved?.isRegistered ?? false);
  ...
  const handleSaveDraft = async () => {
    ...
    const content: SavedContent & { [WIZARD_CASE_TYPE_KEY]: string } = {
      ...
      verificationPlace,
      documentEntries,
      [WIZARD_CASE_TYPE_KEY]: 'ct-suit-cancellation-of-document',
    };
    try {
      if (caseId && draftId) {
        ...
        const created = await casesClient.createCase(
          {
            title: `${plaintiffName || 'Plaintiff'} vs. ${defendantName || 'Defendant'} — Suit for Cancellation of Document`,
  ...
  const citationMatches = findFixedCaseTypeCitation('ct-suit-cancellation-of-document');
  const caseLawMatches = findFixedCaseTypeCaseLaw('ct-suit-cancellation-of-document');
  ...
  const documentRef = documentDetails.trim() || '[describe the document — its nature, date, and registration details, if any]';
  ...
  const draftSections: DraftSection[] = [
    {
      ...
      paragraphs: [
        toThatClause(
          `${documentRef} is ${
            groundType === 'void' ? 'void' : 'voidable'
          } as against the Plaintiff, and the Defendant is a party to, or claims some right under, the said document`
        ),
      ],
      incomplete: !documentDetails.trim(),
    },
    {
      heading: groundType === 'void' ? 'Grounds — the document is void' : 'Grounds — the document is voidable',
      paragraphs: [toThatClause(groundsNarrative.trim() || '[describe the specific defect — e.g. forgery, fraud, undue influence, lack of capacity, or want of authority — that renders the document void/voidable]')],
      incomplete: !groundsNarrative.trim(),
      role: 'grounds',
    },
    {
      heading: 'Apprehended injury',
      paragraphs: [
        toThatClause(
          `the Plaintiff has a reasonable apprehension that, if the said document is left outstanding, it may cause the Plaintiff serious injury, in that ${
            apprehendedInjury.trim() || '[describe the serious injury that would result if the document is not cancelled]'
          }`
        ),
      ],
      incomplete: !apprehendedInjury.trim(),
    },
    {
      heading: 'Facts',
      paragraphs: [toThatClause(factsNarrative.trim() || '[Describe, in chronological order, the circumstances leading to the execution of the document and how the Plaintiff came to learn of the defect]')],
    ...
    },
    {
      ...
      paragraphs: [
        toThatClause(
          `this suit is filed within three years of ${
            knowledgeDate ? new Date(knowledgeDate).toLocaleDateString('en-IN', { day: 'numeric', month: 'long', year: 'numeric' }) : '[date]'
          }, being the date the facts entitling the Plaintiff to have the document cancelled first became known to the Plaintiff, and is therefore within the time prescribed by Article 59 of the Limitation Act, 1963`
        ),
      ],
      incomplete: !knowledgeDate,
    },
    ...
    },
    {
      heading: 'Prayer',
      paragraphs: [
        `It is therefore most respectfully prayed that this Hon'ble Court may be pleased to adjudge ${documentRef} as ${
          groundType === 'void' ? 'void' : 'voidable'
        } and order the same to be delivered up and cancelled${
          isRegistered
            ? ", and direct that a copy of the decree be sent to the officer in whose office the document is registered, for the fact of its cancellation to be noted against the document's registration"
            : ''
  ...
  return (
    <div>
      ...
      >
        ...
        {step === 2 && (
          <div>
            ...
            <label className="form-field" style={{ marginTop: 'var(--space-4)' }}>
              <span>Describe the document to be cancelled (type, date, registration details if any)</span>
              <textarea
                className="facts-textarea"
                rows={3}
                value={documentDetails}
                onChange={(e) => setDocumentDetails(e.target.value)}
                placeholder="e.g. a sale deed dated 10.01.2024, registered as Document No. 456/2024 with the Sub-Registrar, Pune"
              />
            </label>
            <label className="form-field" style={{ marginTop: 'var(--space-4)' }}>
              <span>
                <input type="checkbox" checked={isRegistered} onChange={(e) => setIsRegistered(e.target.checked)} style={{ marginRight: 'var(--space-2)' }} />
                This document is registered
              </span>
            </label>
          </div>
        )}
              ...
              <label className="form-field">
                <span>
                  <input
                    type="radio"
                    name="ground-type"
                    checked={groundType === 'voidable'}
                    onChange={() => setGroundType('voidable')}
                    style={{ marginRight: 'var(--space-2)' }}
                  />
                  Voidable — valid unless and until set aside (e.g. obtained by fraud, undue influence, or misrepresentation)
                </span>
              </label>
              <label className="form-field">
                <span>
                  <input
                    type="radio"
                    name="ground-type"
                    checked={groundType === 'void'}
                    onChange={() => setGroundType('void')}
                    style={{ marginRight: 'var(--space-2)' }}
                  />
                  Void — a nullity from the start (e.g. forged, or executed by someone without any right/authority to do so)
                </span>
              </label>
            </div>
            <label className="form-field">
              <span>What specifically makes the document void/voidable?</span>
              <textarea
                className="facts-textarea"
                rows={4}
                value={groundsNarrative}
                onChange={(e) => setGroundsNarrative(e.target.value)}
                placeholder="Describe the specific defect and how you discovered it"
              />
            </label>
            <label className="form-field" style={{ marginTop: 'var(--space-4)' }}>
              <span>What serious injury would result if this document is left outstanding, uncancelled?</span>
              <textarea
                className="facts-textarea"
                rows={3}
                value={apprehendedInjury}
                onChange={(e) => setApprehendedInjury(e.target.value)}
                placeholder="e.g. it clouds the Plaintiff's title and prevents the Plaintiff from selling or mortgaging the property"
              />
            </label>
            <p className="step-help">
              A document that's void ab initio is already a nullity — a plain declaration suit can be enough for
              that. A voidable document remains legally effective unless a court actually cancels it, which is what
              this suit is chiefly for (<em>Prem Singh v. Birbal</em>, (2006) 5 SCC 353).
            </p>
          </div>
        )}
        ...
        {step === 4 && (
          <div>
            <h3 className="step-heading">Facts and limitation</h3>
            <label className="form-field">
              <span>Facts — background of the document and how the defect came to light</span>
              <textarea
                className="facts-textarea"
                rows={6}
                value={factsNarrative}
                onChange={(e) => setFactsNarrative(e.target.value)}
                placeholder="Describe the events in chronological order, including how and when you discovered the fraud/defect"
              />
            </label>
            <label className="form-field" style={{ marginTop: 'var(--space-4)', maxWidth: 320 }}>
              <span>Date you learnt of the facts entitling you to cancel this document</span>
              <input type="date" value={knowledgeDate} onChange={(e) => setKnowledgeDate(e.target.value)} />
            </label>
            <p className="step-help">
              Your 3-year limitation period under Article 59 of the Limitation Act, 1963 runs from this date of
              knowledge — not from the date of the document itself.
            </p>
            <label className="form-field" style={{ marginTop: 'var(--space-4)', maxWidth: 320 }}>
        ...
        {step === 6 && (
          <div>
            ...
            <p className="step-help">
              Add each document you're annexing — typically a copy of the document sought to be cancelled, and any
              evidence of the fraud/defect — in the order it will be paginated.
            </p>
        ...
        {step === 7 && (
          <div>
            ...
            <DraftDocument title="Suit for Cancellation of Document — Index" causeTitleHtml={indexCauseTitleHtml} sections={indexSections} />
            ...
            <DraftDocument
              ...
              subtitle={`Suit for Cancellation of Document — ${plaintiffName || '[Plaintiff]'} vs. ${defendantName || '[Defendant]'}`}
              causeTitleHtml={causeTitleHtml}
              sections={applyJudgeStyleToSections(draftSections, judgeStyleProfile)}
            />
            <h4 style={{ marginTop: 'var(--space-6)' }}>Part III — Affidavit</h4>
            <DraftDocument title="Suit for Cancellation of Document — Affidavit" causeTitleHtml={affidavitCauseTitleHtml} sections={affidavitSections} />
